<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $session = \Stripe\Checkout\Session::create([
            ...
            'mode' => 'payment',
            'success_url' => 'http://localhost/rentronics/successcopy.php?session_id={CHECKOUT_SESSION_ID}',
    ...
    <div class="container-fluid bg-white p-0" style="">
        ...
        <div class="page-wrapper-payment">
            ...
            <form action="" method="post" class="paymentform">
                ...
                <select name="agentName" id="agentName" required>
                ...
                <select name="roomId" id="roomId" required>
                    <?php foreach ($dataSheet2 as $row): ?>
                        <option class="roomOption property-<?php echo $row[1]; ?>" value="<?php echo $row[0]; ?>">( <?php echo $row[1]; ?> ) <?php echo $row[0]; ?> - <?php echo $row[3]; ?></option>
                    <?php endforeach; ?>
                </select>
...
<script>
    function filterRooms() {
        ...
        var rooms = document.getElementsByClassName("roomOption");
        
        for (var i = 0; i < rooms.length; i++) {
            var roomProperty = rooms[i].classList[1]; // Assuming the class contains the property ID

            if (roomProperty === "property-" + propertyId) {
